<template>
    <div id="app">
        ...
        <page-adress breadcrumbTittle="Fornecedor"></page-adress>
    
        <v-app id="inspire">
            <v-form>
                <v-data-table :headers="headers" :items="fornecedores" class="elevation-1">
                    <template v-slot:top>
                        <v-toolbar flat color="white">
                            <v-text-field
                              @keyup="procurarfornecedor"
                              v-model="fornecedoresearched"
                              label="Pesquisar Fornecedor ou Veiculo"
                              single-line
                            ></v-text-field>
                            <v-spacer></v-spacer>
                    ...
                    <v-dialog v-model="dialog" max-width="500px">
                            <template v-slot:activator="{ on }">
                              <v-btn color="primary" dark class="mb-2" v-on="on">Novo Fornecedor</v-btn>
                            </template>
                <v-card>
                  <v-card-title>
...
export default {
    ...
    computed: {
        ...
        disableButton: function() {
            if (this.editedItem.email.length < 1) {
                return this.editedItem.nome.length < 1 || this.editedItem.telefone < 1;
            }
            if (
    ...
    created() {
        Fire.$on("searching", () => {
            let query = this.fornecedoresearched;

            axios.get("api/findfornecedor?query=" + query).then(response => {
    ...
    methods: {
        ...
        initialize() {
            this.$Progress.start();
            this.fornecedoresearched = "";
            axios.get("api/fornecedor").then(response => {
                this.fornecedores = response.data.data;
            });
        ...
        save() {
            if (this.editedIndex > -1) {
                axios
                    .put("/api/fornecedor/" + this.editedItem.id, this.editedItem)
                    .then(() => {
                    ...
                    .catch(e => {
                        console.log("errorr on update Fornecedor component", e);
                    });
            } else {
                axios
                    .post("/api/fornecedor", this.editedItem)
                    .then(() => {
                    ...
                    .catch(e => {
                        console.log("errorr on create Fornecedor component", e);
                    });
            }
